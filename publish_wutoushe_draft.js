...
try {
  wechatConfig = JSON.parse(fs.readFileSync(
    path.join(process.env.HOME || process.env.USERPROFILE, '.workbuddy/wechat/wutoushe_config.json'), 'utf-8'
  ));
...
async function main() {
  try {
    console.log('=== 雾透科技公众号' + (draftOnly ? '草稿箱推送' : '自动发布') + ' ===');
    ...
    const ghPagesBase = 'https://wyb123123123.github.io/wutoushe-articles/';
